...
public class ReflectUtils {
    ...
    public static <E> E invokeGetter(Object obj, String propertyName) throws NoSuchMethodException {
        Object object = obj;
        for (String name : ExtStrUtils.split(propertyName, ExtStrUtils.PERIOD)) {
            String getterMethodName = GETTER_PREFIX + ExtStrUtils.capitalize(name);
    ...
    public static <E> void invokeSetter(Object obj, String propertyName, E value) throws NoSuchMethodException {
        Object object = obj;
        String[] names = ExtStrUtils.split(propertyName, ExtStrUtils.PERIOD);
        for (int i = 0; i < names.length; i++) {
            if (i < names.length - 1) {
                String getterMethodName = GETTER_PREFIX + ExtStrUtils.capitalize(names[i]);
                object = invokeMethod(object, getterMethodName, new Class[]{}, new Object[]{});
            } else {
                String setterMethodName = SETTER_PREFIX + ExtStrUtils.capitalize(names[i]);
     ...
     */
    public static <E> void setFieldValue(Object obj, String fieldName, E value) throws NoSuchFieldException {
        Field field = getAccessibleField(obj.getClass(), fieldName);
        try {
            field.set(obj, value);
        } catch (IllegalAccessException e) {
     ...
     */
    @SuppressWarnings("unchecked")
    public static <E> E invokeMethodByName(Object obj, String methodName, Object[] args) throws NoSuchMethodException {
            ...
            for (int i = 0; i < cs.length; i++) {
                if (args[i] != null && !args[i].getClass().equals(cs[i])) {
                    if (cs[i] == String.class) {
                        ...
                        if (ExtStrUtils.endsWith((String)args[i], ".0")) {
                            args[i] = ExtStrUtils.substringBefore((String)args[i], ".0");
                        }
                        ...
                        if (args[i] instanceof String) {
                            args[i] = TimeUtils.parseDate(args[i].toString());
                        } else {
                            args[i] = new Date(((Double)args[i]).longValue());
                        }
    ...
    public static Field getAccessibleField(Class<?> clazz, String fieldName) throws NoSuchFieldException {
        ...
        throw new NoSuchFieldException("No such accessible field: " + clazz.getSimpleName() + ExtStrUtils.PERIOD + fieldName);
    }
    ...
    public static Method getAccessibleMethod(Class<?> clazz, String methodName,
        ...
        throw new NoSuchMethodException("No such accessible method: "
                + clazz.getSimpleName() + ExtStrUtils.PERIOD + methodName
                + MessageFormat.format("({0})", ExtStrUtils.join(parameterTypes, ExtStrUtils.COMMA)));
    }
     ...
     * 用于方法需要被多次调用的情况. 先使用本函数先取得Method,然后调用Method.invoke(Object obj, Object... args)
     */
    public static Method getAccessibleMethodByName(Object obj, String methodName, int argsNum) throws NoSuchMethodException {
        // 为空不报错。直接返回 null
        if (obj == null) {
            return null;
        }
        ...
        throw new NoSuchMethodException("No such accessible method: "
                + obj.getClass().getSimpleName() + ExtStrUtils.PERIOD + methodName
                + MessageFormat.format("({0})", argsNum));
    }
