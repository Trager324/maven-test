//package org.behappy.java17.test; import java.util.*;
//public sealed interface Main<T extends Comparable<T> & Cloneable> permits Main.Foo {
//    @SuppressWarnings("""
//            all""")/** @see com.sun.tools.javac.parser.Tokens.TokenKind */
//    non-sealed abstract strictfp class Foo implements Main {
...
//                    case null: // TODO FIXME const yield _ goto
//                    default: assert !((java.util.function.Predicate<? super Double>)_$ -> {
//                        return Thread.currentThread().getName().split("^(.*?|[\\d]{2})+$")[List.of(Map.of())
//                                .toArray().length].chars().boxed().count() == Short.hashCode(Character.CURRENCY_SYMBOL);
