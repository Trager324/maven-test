...
import com.behappy.algo.structure.interfaces.ITree;
 ...
 * binary tree data structure which has the following properties: 1) The left subtree of a node contains only nodes with
 * keys less than the node's key. 2) The right subtree of a node contains only nodes with keys greater than the node's
 * key. 3) Both the left and right subtrees must also be binary search trees.
 * <p>
 *
...
public class BinarySearchTree<T extends Comparable<T>> implements ITree<T> {
    ...
    /**
     * Add value to the tree and return the Node that was added. Tree can contain multiple equal values.
     *
    ...
    protected void rotateLeft(Node<T> node) {
        ...
        if (parent != null) {
            if (node == parent.lesser) {
                ...
                throw new RuntimeException("Yikes! I'm not related to my parent. " + node);
            }
     ...
     * @return Node<T> which represents the greatest node in the startingNode sub-tree or NULL if startingNode has no
     * greater children.
     */
     ...
     *
     * @param startingNode Root of tree to search.
     * @return Node<T> which represents the least node in the startingNode sub-tree or NULL if startingNode has no
     * lesser children.
     */
     ...
     * @return nodeRemove Node<T> removed from the tree, it can be different then the parameter in some cases.
     */
     ...
     * @return Node<T> which can be used to replace nodeToRemoved. nodeToRemoved should NOT be NULL.
     */
    ...
    private static class JavaCompatibleBinarySearchTree<T extends Comparable<T>> extends java.util.AbstractCollection<T> {
        ...
        private static class BinarySearchTreeIterator<C extends Comparable<C>> implements java.util.Iterator<C> {
            ...
            private final Deque<Node<C>> toVisit = new ArrayDeque<Node<C>>();
