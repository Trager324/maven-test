...
/**
 * A radix trie or radix tree is a space-optimized trie data structure where each node with only one child is merged
 * with its child. The result is that every internal node has at least two children. Unlike in regular tries, edges can
 * be labeled with sequences of characters as well as single characters. This makes them much more efficient for small
 * sets (especially if the strings are long) and for sets of strings that share long prefixes. This particular radix
 * tree is used to represent an associative array.
 * <p>
...
public class RadixTrie<K extends CharSequence, V> implements PatriciaTrie.INodeCreator, IMap<K, V> {
    ...
    protected static final class RadixNode<K extends CharSequence, V> extends PatriciaTrie.Node {
        ...
        private RadixNode(PatriciaTrie.Node node, V value) {
        ...
        private RadixNode(PatriciaTrie.Node parent, char[] string, boolean type) {
            super(parent, string, type);
        }

        private RadixNode(PatriciaTrie.Node parent, char[] string, boolean type, V value) {
            super(parent, string, type);
            this.value = value;
        }
