 ...
 * deletions in logarithmic time. The B-tree is a generalization of a binary search tree in that a node can have more
 * than two children. Unlike self-balancing binary search trees, the B-tree is optimized for systems that read and write
...
public class BTree<T extends Comparable<T>> implements ITree<T> {
    ...
    private static class Node<T extends Comparable<T>> {
        ...
        private T[] keys = null;
        private final Comparator<Node<T>> comparator = new Comparator<Node<T>>() {
            @Override
