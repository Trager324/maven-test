...
import com.behappy.algo.structure.interfaces.IMap;
...
/**
 * A hash array mapped trie (HAMT) is an implementation of an associative array that combines the characteristics of a
 * hash table and an array mapped trie. It is a refined version of the more general notion of a hash tree.
 * <p>
 * This implementation is 32-bit and steps in 5-bit intervals, maximum tree height of 7.
 * <p>
 *
...
public class HashArrayMappedTrie<K, V> implements IMap<K, V> {
     ...
     * <p>
     * e.g. BITS=5 height=1 value==266 big-endian=100001010 (shifts height*BITS off the right) return=1000 (8 in
     * decimal)
     */
    ...
    /**
     * Convert a big-endian binary string to a little-endian binary string and also pads with zeros to make it "BITS"
     * length.
     * <p>
    ...
    private V put(ArrayNode parent, Node node, byte height, int key, V value) {
        ...
        if (node instanceof KeyValueNode) {
            ...
            return null;
        } else if (node instanceof ArrayNode arrayRoot) {
    ...
    public boolean validate() {
        ...
        if (child instanceof KeyValueNode) {
            KeyValueNode<V> kvChild = (KeyValueNode<V>)child;
            return validate(null, kvChild);
        } else if (child instanceof ArrayNode arrayNode) {
            return validate(null, arrayNode);
        } else {
            return false;
        }
    }
    ...
    private static final class ArrayNode extends Node {
         ...
         * <p>
         * Because each 1 in the bitmap means a child exists, you can use the 1s count to calculate the index of the
        ...
        private void addChild(int position, Node child) {
            boolean overwrite = isSet(position);
    ...
    private static final class KeyValueNode<V> extends Node {
        ...
        @Override
        public String toString() {
            return "key=" + toBinaryString(key) + " value=" + value;
        }
    }
    ...
    private static class JavaCompatibleMap<K, V> extends java.util.AbstractMap<K, V> {
        ...
        private void addToSet(java.util.Set<Entry<K, V>> set, Node node) {
            if (node instanceof KeyValueNode) {
                KeyValueNode<V> kvNode = (KeyValueNode<V>)node;
                set.add(new JavaCompatibleMapEntry<K, V>((K)Integer.valueOf(kvNode.key), kvNode.value));
            } else if (node instanceof ArrayNode arrayNode) {
                for (int i = 0; i < MAX_BITS; i++) {
                    Node child = arrayNode.getChild(i);
