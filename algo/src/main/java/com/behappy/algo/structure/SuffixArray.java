 ...
 * others, in full text indices, data compression algorithms and within the field of bibliometrics.
 * <p>
 *
...
public class SuffixArray {
     ...
     * <p>
     * For radius r: KMR[i] == k, when string[i..i+r-1] is kth r-letter substring of string sorted lexicographically KMR
     * is counted for radius = 1,2,4,8 ... KMR for radius bigger than string length is the inverted suffix array
     */
    ...
    /**
     * Creates KMR array for new radius from nearly inverted array. Elements from inverted array need to be grouped by
     * substring tey represent.
     *
    ...
    private ArrayList<Integer> getKMR(ArrayList<KMRsWithIndex> KMRinvertedList, int length) {
        ...
        for (int i = 0; i < 2 * length; i++)
            KMR.add(Integer.valueOf(-1));

        int counter = 0;
        for (int i = 0; i < length; i++) {
            if (i > 0 && substringsAreEqual(KMRinvertedList, i))
                counter++;
            KMR.set(KMRinvertedList.get(i).index, Integer.valueOf(counter));
        }
    ...
    private boolean substringsAreEqual(ArrayList<KMRsWithIndex> KMRinvertedList, int i) {
        return (!KMRinvertedList.get(i - 1).beginKMR.equals(KMRinvertedList.get(i).beginKMR)) ||
                (!KMRinvertedList.get(i - 1).endKMR.equals(KMRinvertedList.get(i).endKMR));
    }
        ...
        Collections.sort(KMRinvertedList,
                new Comparator<KMRsWithIndex>() {
                    ...
                    public int compare(KMRsWithIndex A, KMRsWithIndex B) {
                        if (!A.beginKMR.equals(B.beginKMR))
                            return A.beginKMR.compareTo(B.beginKMR);
                        if (!A.endKMR.equals(B.endKMR))
    ...
    private ArrayList<Integer> getBasicKMR(int length) {
        ...
        for (int i = 0; i < length; i++)
            result.add(Integer.valueOf(characters[i]));
        for (int i = 0; i < length; i++)
