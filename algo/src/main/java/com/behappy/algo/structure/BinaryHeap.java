...
import com.behappy.algo.structure.interfaces.IHeap;
 ...
 * additional constraints: 1) The shape property: the tree is a complete binary tree; that is, all levels of the tree,
 * except possibly the last one (deepest) are fully filled, and, if the last level of the tree is not complete, the
 * nodes of that level are filled from left to right. 2) The heap property: each node is right than or equal to each of
...
public interface BinaryHeap<T extends Comparable<T>> extends IHeap<T> {
    ...
    class BinaryHeapArray<T extends Comparable<T>> implements BinaryHeap<T> {
        ...
        /**
         * Get the parent index of this index, will return Integer.MIN_VALUE if no parent is possible.
         *
    ...
    class JavaCompatibleBinaryHeapArray<T extends Comparable<T>> extends java.util.AbstractCollection<T> {
    ...
    class JavaCompatibleBinaryHeapTree<T extends Comparable<T>> extends java.util.AbstractCollection<T> {
        ...
        private static class BinaryHeapTreeIterator<C extends Comparable<C>> implements java.util.Iterator<C> {
            ...
            private final Deque<BinaryHeapTree.Node<C>> toVisit = new ArrayDeque<BinaryHeapTree.Node<C>>();
