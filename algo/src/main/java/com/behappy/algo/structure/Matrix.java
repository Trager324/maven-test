...
/**
 * Matrx. This Matrix implementation is designed to be more efficient in cache. A matrix is a rectangular array of
...
public class Matrix<T extends Number> {
    ...
    private final Comparator<T> comparator = new Comparator<>() {
        /**
         * {@inheritDoc}
         */
        @Override
    ...
    /**
     * Matrix with 'rows' number of rows and 'cols' number of columns, populates the double index matrix.
     *
     * @param rows   Number of rows in Matrix.
     * @param cols   Number of columns in Matrix.
    ...
    public Matrix<T> identity() throws Exception {
        ...
        if (element instanceof BigDecimal) {
            ...
            one = (T)Long.valueOf(1);
        } else if (element instanceof Double) {
            zero = (T)Double.valueOf(0);
            one = (T)Double.valueOf(1);
        } else if (element instanceof Float) {
            zero = (T)Float.valueOf(0);
            one = (T)Float.valueOf(1);
        } else {
            zero = (T)Integer.valueOf(0);
            one = (T)Integer.valueOf(1);
        }
        ...
        final Matrix<T> identityMatrix = new Matrix<>(this.rows, this.cols, array);
        for (int i = 0; i < this.rows; ++i) {
    ...
    public Matrix<T> add(Matrix<T> input) {
        Matrix<T> output = new Matrix<>(this.rows, this.cols);
    ...
    public Matrix<T> multiply(Matrix<T> input) {
        Matrix<T> output = new Matrix<>(this.rows, input.cols);
