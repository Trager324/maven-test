...
/**
 * The treap and the randomized binary search tree are two closely related forms of binary search tree data structures
 * that maintain a dynamic set of ordered values and allow binary searches among the values. After any sequence of
 * insertions and deletions of values, the shape of the tree is a random variable with the same probability distribution
 * as a random binary tree; in particular, with high probability its height is proportional to the logarithm of the
 * number of values, so that each search, insertion, or deletion operation takes logarithmic time to perform.
 * <p>
 *
...
public class Treap<T extends Comparable<T>> extends BinarySearchTree<T> {
    ...
    private static class TreapNode<T extends Comparable<T>> extends Node<T> {
        ...
        public String toString() {
            ...
            builder.append("\n");
            if (lesser != null) builder.append("left=").append(lesser).append("\n");
            if (greater != null) builder.append("right=").append(greater).append("\n");
