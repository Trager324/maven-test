...
/**
 * A Treap is a self-balancing binary search tree that uses randomization to maintain a low height. In this version, it
 * is used emulate the operations of an array and linked list.
 * <p>
 * Time Complexity: Assuming the join/merge functions have constant complexity. add(value), add(index,value),
 * remove(index), set(index,value), get(index) all have O(log N). remove(value), get(value), contains(value) all have
 * O(N).
 * <p>
