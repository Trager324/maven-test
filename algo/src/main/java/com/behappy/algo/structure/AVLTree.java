...
/**
 * An AVL tree is a self-balancing binary search tree, and it was the first such data structure to be invented. In an
 * AVL tree, the heights of the two child subtrees of any node differ by at most one. AVL trees are often compared with
 * red-black trees because they support the same set of operations and because red-black trees also take O(log n) time
 * for the basic operations. Because AVL trees are more rigidly balanced, they are faster than red-black trees for
 * lookup intensive applications. However, red-black trees are faster for insertion and removal.
 * <p>
 *
...
public class AVLTree<T extends Comparable<T>> extends BinarySearchTree<T> {
     ...
     */
    public AVLTree() {
        this.creator = new BinarySearchTree.INodeCreator<>() {
            /**
             * {@inheritDoc}
             */
            @Override
    ...
    protected static class AVLNode<T extends Comparable<T>> extends Node<T> {
         ...
         *
         * @return An integer representing the balance factor for this node. It will be negative if the lesser branch is
         * longer than the greater branch.
         */
    ...
    protected static class AVLTreePrinter {
        ...
        private static <T extends Comparable<T>> String getString(AVLNode<T> node, String prefix, boolean isTail) {
            ...
            builder.append(prefix).append(isTail ? "└── " : "├── ").append("(").append(node.height).append(") ").append(node.id).append("\n");
