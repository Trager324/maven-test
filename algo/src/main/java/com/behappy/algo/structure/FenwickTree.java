 ...
 * manipulation of the prefix sums of a table of values. Fenwick trees primarily solve the problem of balancing prefix
 * sum calculation efficiency with element modification efficiency.
 * <p>
 * This class is meant to be somewhat generic, all you'd have to do is extend the Data abstract class to store your
 * custom data. I've included a range sum implementations.
 * <p>
 *
...
public class FenwickTree<D extends FenwickTree.Data> {
    ...
    protected static class FenwickTreePrinter {
        ...
        public static <D extends Data> String getString(FenwickTree<D> tree) {
            ...
            String builder = "└── dummy \n" +
                    getString(tree, 0, tree.array.length, "", true);
            return builder;
        }
        ...
        private static <D extends Data> String getString(FenwickTree<D> tree, int start, int end, String prefix, boolean isTail) {
            ...
            int next = start + 1;
            final List<Integer> children = new ArrayList<>(2);
            while (next < end) {
                children.add(next);
                next = next(next);
            }
