...
/**
 * Graph. Could be directed or undirected depending on the TYPE enum. A graph is an abstract representation of a set of
 * objects where some pairs of the objects are connected by links.
 * <p>
 *
...
public class Graph<T extends Comparable<T>> {
    ...
    private final List<Vertex<T>> allVertices = new ArrayList<Vertex<T>>();
    private final List<Edge<T>> allEdges = new ArrayList<Edge<T>>();
     ...
     * <p>
     * NOTE: Duplicate vertices and edges ARE allowed. NOTE: Copies the vertex and edge objects but does NOT store the
     * Collection parameters itself.
     *
    ...
    public static class Edge<T extends Comparable<T>> implements Comparable<Edge<T>> {
        ...
        @Override
        public String toString() {
            String builder = "[ " + from.value + "(" + from.weight + ") " + "]" + " -> " +
    ...
    public static class CostVertexPair<T extends Comparable<T>> implements Comparable<CostVertexPair<T>> {
        ...
        @Override
        public boolean equals(Object e1) {
            if (!(e1 instanceof final CostVertexPair<?> pair))
                return false;
    ...
    public static class CostPathPair<T extends Comparable<T>> {
        ...
        public boolean equals(Object obj) {
            if (!(obj instanceof final CostPathPair<?> pair))
                return false;
