...
import com.behappy.algo.structure.interfaces.ISet;

import java.util.Random;

/**
 * Skip List. A skip list is a data structure for storing a sorted list of items using a hierarchy of linked lists that
 * connect increasingly sparse subsequences of the items. These auxiliary lists allow item lookup with efficiency
 * comparable to balanced binary search trees.
 * <p>
 *
...
public class SkipList<T extends Comparable<T>> implements ISet<T> {
    ...
    protected Node<T> addValue(T value) {
        ...
        if (head == null) {
            // new list
            Node<T> node = null;
            if (creator == null) node = new Node<>(MAX, value);
            ...
            int level = getRandom();
            Node<T> node = null;
            if (creator == null) node = new Node<>(level, value);
    ...
    private NodeLevelPair<T> getPredecessor(T value) {
        ...
        while (next != null) {
            ...
            if (comp == 0) {
                ...
                NodeLevelPair<T> pair = new NodeLevelPair<>(level, node);
                return pair;
            } else if (comp >= 1) {
    ...
    public java.util.Set<T> toSet() {
        return (new JavaCompatibleSkipList<>(this));
    }
    ...
    public static class JavaCompatibleSkipList<T extends Comparable<T>> extends java.util.AbstractSet<T> {
        ...
        public java.util.Iterator<T> iterator() {
            return (new SkipListListIterator<>(list));
        }
