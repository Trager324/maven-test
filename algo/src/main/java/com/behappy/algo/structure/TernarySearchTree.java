...
/**
 * In computer science, a ternary search tree is a type of trie (sometimes called a prefix tree) where nodes are
 * arranged in a manner similar to a binary search tree, but with up to three children rather than the binary tree's
 * limit of two.
 * <br>
 * <a href="https://www.cs.upc.edu/~ps/downloads/tst/tst.html">This implementation is based on Jon Bentley and Bob
 * Sedgewick's paper.</a>
 * <p>
 *
...
public class TernarySearchTree<C extends CharSequence> implements ITree<C> {
    ...
    protected static class TreePrinter {
        ...
        protected static String getString(Node node, String prefix, String previousString, boolean isTail) {
            ...
            builder.append(prefix + (isTail ? "└── " : "├── ") + ((node.isWord) ?
                    ("(" + node.character + ") " + string + node.character)
                    :
                    node.character) + "\n"
            );
            if (node.loKid != null)
            ...
            if (node.kid != null)
                builder.append(getString(node.kid, prefix + (isTail ? "    " : "│   "), string + node.character, false));
            if (node.hiKid != null)
