...
/**
 * A trie, or prefix tree, is an ordered tree data structure that is used to store an associative array where the keys
 * are usually strings.
 * <p>
...
public class Trie<C extends CharSequence> implements ITree<C> {
    ...
    protected C remove(Node node) {
        ...
        if (node.childrenSize > 0) {
            ...
            // represents a string
            while (previous != null && !previous.isWord && previous.childrenSize == 0) {
    ...
    private boolean validate(Node node, String string, java.util.Set<C> keys) {
        String s = string + node.character;
    ...
    protected static class Node {
        ...
        public String toString() {
            ...
            if (isWord) builder.append("Node=").append(isWord).append("\n");
            for (int i = 0; i < childrenSize; i++) {
                Node c = children[i];
