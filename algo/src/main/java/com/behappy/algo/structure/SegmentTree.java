package com.behappy.algo.structure;
...
/**
 * Segment tree using objects and pointers. A segment tree is a tree data structure for storing intervals, or segments.
 * It allows querying which of the stored segments contain a given point. It is, in principle, a static structure; that
 * is, its content cannot be modified once the structure is built.
 * <p>
 * This class is meant to be somewhat generic, all you'd have to do is extend the Data abstract class to store your
 * custom data. I've also included a range minimum, range maximum, range sum, and interval stabbing implementations.
 * <p>
 *
...
public abstract class SegmentTree<D extends SegmentTree.Data> {
    ...
    public abstract static class Data implements Comparable<Data> {
        ...
        public int compareTo(Data d) {
            return Long.compare(this.end, d.end);
        }

        /**
         * Data structure representing points in the x,y space and their location in the quadrants.
         */
        public static final class QuadrantData extends Data {
            ...
            public boolean equals(Object obj) {
                if (!(obj instanceof QuadrantData data))
                    return false;
        ...
        public static final class RangeMaximumData<N extends Number> extends Data {
            ...
            private void combined(RangeMaximumData<N> data) {
                if (this.maximum == null && data.maximum == null) {} else if (this.maximum != null && data.maximum == null) {} else if (this.maximum == null && data.maximum != null)
                    this.maximum = data.maximum;
                else {
                    /* TODO: This is ugly */
                    if (this.maximum instanceof BigDecimal || data.maximum instanceof BigDecimal) {
                        if (((BigDecimal)data.maximum).compareTo(((BigDecimal)this.maximum)) > 0)
                            this.maximum = data.maximum;
                    } else if (this.maximum instanceof BigInteger || data.maximum instanceof BigInteger) {
                        if (((BigInteger)data.maximum).compareTo(((BigInteger)this.maximum)) > 0)
                            this.maximum = data.maximum;
                    } else if (this.maximum instanceof Long || data.maximum instanceof Long) {
                        if (((Long)data.maximum).compareTo(((Long)this.maximum)) > 0)
                            this.maximum = data.maximum;
                    } else if (this.maximum instanceof Double || data.maximum instanceof Double) {
                        if (((Double)data.maximum).compareTo(((Double)this.maximum)) > 0)
                            this.maximum = data.maximum;
                    } else if (this.maximum instanceof Float || data.maximum instanceof Float) {
                        if (((Float)data.maximum).compareTo(((Float)this.maximum)) > 0)
                            this.maximum = data.maximum;
                    } else {
                        // Integer
                        if (((Integer)data.maximum).compareTo(((Integer)this.maximum)) > 0)
                            this.maximum = data.maximum;
                    }
                }
            }
        ...
        public static final class RangeMinimumData<N extends Number> extends Data {
            ...
            private void combined(RangeMinimumData<N> data) {
                if (this.minimum == null && data.minimum == null) {} else if (this.minimum != null && data.minimum == null) {} else if (this.minimum == null && data.minimum != null)
                    this.minimum = data.minimum;
                else {
                    /* TODO: This is ugly */
                    if (this.minimum instanceof BigDecimal || data.minimum instanceof BigDecimal) {
                        if (((BigDecimal)data.minimum).compareTo(((BigDecimal)this.minimum)) < 0)
                            this.minimum = data.minimum;
                    } else if (this.minimum instanceof BigInteger || data.minimum instanceof BigInteger) {
                        if (((BigInteger)data.minimum).compareTo(((BigInteger)this.minimum)) < 0)
                            this.minimum = data.minimum;
                    } else if (this.minimum instanceof Long || data.minimum instanceof Long) {
                        if (((Long)data.minimum).compareTo(((Long)this.minimum)) < 0)
                            this.minimum = data.minimum;
                    } else if (this.minimum instanceof Double || data.minimum instanceof Double) {
                        if (((Double)data.minimum).compareTo(((Double)this.minimum)) < 0)
                            this.minimum = data.minimum;
                    } else if (this.minimum instanceof Float || data.minimum instanceof Float) {
                        if (((Float)data.minimum).compareTo(((Float)this.minimum)) < 0)
                            this.minimum = data.minimum;
                    } else {
                        // Integer
                        if (((Integer)data.minimum).compareTo(((Integer)this.minimum)) < 0)
                            this.minimum = data.minimum;
                    }
                }
            }
        ...
        public static final class RangeSumData<N extends Number> extends Data {
            ...
            private void combined(RangeSumData<N> data) {
                if (this.sum == null && data.sum == null) {} else if (this.sum != null && data.sum == null) {} else if (this.sum == null && data.sum != null)
                    this.sum = data.sum;
                else {
         ...
         */
        public static final class IntervalData<O extends Comparable<O>> extends Data {
            ...
            @Override
            public Data copy() {
                final Set<O> listCopy = new TreeSet<>(set);
    ...
    protected static class SegmentTreePrinter {
        ...
        private static <D extends Data> String getString(Segment<D> segment, String prefix, boolean isTail) {
            ...
            builder.append(prefix).append(isTail ? "└── " : "├── ").append(segment.toString()).append("\n");

            final List<Segment<D>> children = new ArrayList<>();
            if (segment.segments != null) {
                children.addAll(Arrays.asList(segment.segments));
            }
     ...
     * segments. This structure is efficient when you need to store values associated with 1 dimensional segments that
     * never overlap with each other. The end points of stored segments are inclusive, that is, when a segment spans
     * from 2 to 6, an arbitrary point x within that segment can take a value of 2 <= x <= 6.
     */
    public static final class FlatSegmentTree<D extends Data> extends SegmentTree<D> {
        ...
        public FlatSegmentTree(List<D> data, int minLength) {
            ...
            // Check for gaps
            final List<NonOverlappingSegment<D>> segments = new ArrayList<>();
            for (int i = 0; i < data.size(); i++) {
                if (i < data.size() - 1) {
                    final D d1 = data.get(i);
                    final NonOverlappingSegment<D> s1 = new NonOverlappingSegment<>(minLength, d1.start, d1.end, d1);
                    segments.add(s1);
                    ...
                    if (d2.start - d1.end > 1) {
                        ...
                        d3.start = d1.end + 1;
                        d3.end = d2.start - 1;
                        final NonOverlappingSegment<D> s3 = new NonOverlappingSegment<>(minLength, d3.start, d3.end, (D)d3);
                        segments.add(s3);
                    }
                } else {
                    final D d1 = data.get(i);
        ...
        protected static final class NonOverlappingSegment<D extends Data> extends Segment<D> {
            ...
            private final Set<Segment<D>> set = new TreeSet<>();
            ...
            private static <D extends Data> Segment<D> createFromList(int minLength, List<NonOverlappingSegment<D>> segments, long start, int length) {
            ...
            public D query(long startOfQuery, long endOfQuery) {
                if (startOfQuery == this.start && endOfQuery == this.end) {
                    if (this.data == null)
                        return null;
                    return ((D)this.data.query(startOfQuery, endOfQuery));
                }
                ...
                if (this.hasChildren()) {
                    if (startOfQuery <= this.getLeftChild().end && endOfQuery > this.getLeftChild().end) {
                        final D q1 = this.getLeftChild().query(startOfQuery, getLeftChild().end);
                        final D q2 = this.getRightChild().query(getRightChild().start, endOfQuery);
                        if (q1 == null && q2 == null)
                            return null;
                        if (q1 != null && q2 == null)
                            return q1;
    ...
    public static final class DynamicSegmentTree<D extends Data> extends SegmentTree<D> {
        ...
        private static final Comparator<OverlappingSegment<?>> END_COMPARATOR = new Comparator<OverlappingSegment<?>>() {
            ...
            @Override
            public int compare(OverlappingSegment<?> arg0, OverlappingSegment<?> arg1) {
                return Long.compare(arg0.end, arg1.end);
            }
        };
        ...
        public DynamicSegmentTree(List<D> data, int minLength) {
            ...
            // Check for gaps
            final List<OverlappingSegment<D>> segments = new ArrayList<>();
            for (int i = 0; i < data.size(); i++) {
                if (i < data.size() - 1) {
                    final Data d1 = data.get(i);
                    final OverlappingSegment<D> s1 = new OverlappingSegment<>(minLength, d1.start, d1.end, (D)d1);
                    segments.add(s1);
                    ...
                    if (d2.start - d1.end > 1) {
                        ...
                        final OverlappingSegment<D> s3 = new OverlappingSegment<>(minLength, d3.start, d3.end, (D)d3);
                        segments.add(s3);
                    }
                } else {
                    final Data d1 = data.get(i);
            ...
            // First start first
            segments.sort(START_COMPARATOR);
        ...
        protected static final class OverlappingSegment<D extends Data> extends Segment<D> {
            ...
            private final Set<Segment<D>> range = new HashSet<>();
            ...
            private static <D extends Data> Segment<D> createFromList(int minLength, List<OverlappingSegment<D>> segments, long start, int length) {
