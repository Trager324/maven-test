...
/**
 * A Patricia trie (radix tree) is a space-optimized trie data structure where each non-terminating (black) node with
 * only one child is merged with its child. The result is that every internal non-terminating (black) node has at least
...
public class PatriciaTrie<C extends CharSequence> implements ITree<C> {
     ...
     * @return Node which represents the sequence in the trie or NULL if the sequence already exists.
     */
    ...
    protected void remove(Node node) {
        ...
        if (node.getChildrenSize() == 0) {
            ...
            String builder = String.valueOf(node.string) +
                    String.valueOf(child.string);
            child.string = builder.toCharArray();
            child.parent = parent;
