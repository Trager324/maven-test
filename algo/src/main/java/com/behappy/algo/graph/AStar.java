...
public class AStar<T extends Comparable<T>> {
    ...
    public List<Graph.Edge<T>> aStar(Graph<T> graph, Graph.Vertex<T> start, Graph.Vertex<T> goal) {
        ...
        final Comparator<Graph.Vertex<T>> comparator = new Comparator<>() {
            /**
             * {@inheritDoc}
             */
            @Override
    ...
    /**
     * Default distance is the edge cost. If there is no edge between the start and next then it returns
     * Integer.MAX_VALUE;
     */
