 ...
 * allows some of the edge weights to be negative numbers, but no negative-weight cycles may exist.
 * <p>
...
public class Johnson {
    ...
    public static Map<Graph.Vertex<Integer>, Map<Graph.Vertex<Integer>, List<Graph.Edge<Integer>>>> getAllPairsShortestPaths(Graph<Integer> g) {
        ...
        final Graph<Integer> graph = new Graph<>(g);
        final Graph.Vertex<Integer> connector = new Graph.Vertex<>(Integer.MAX_VALUE);
        ...
        for (Graph.Vertex<Integer> v : graph.getVertices()) {
            ...
            final Graph.Edge<Integer> edge = new Graph.Edge<>(0, connector, graph.getVertices().get(indexOfV));
        ...
        graph.getVertices().remove(connector);
        for (Graph.Edge<Integer> e : connector.getEdges()) {
            graph.getEdges().remove(e);
        }

        final Map<Graph.Vertex<Integer>, Map<Graph.Vertex<Integer>, List<Graph.Edge<Integer>>>> allShortestPaths = new HashMap<>();
        for (Graph.Vertex<Integer> v : graph.getVertices()) {
