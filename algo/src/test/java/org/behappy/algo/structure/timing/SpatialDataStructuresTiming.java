...
import org.behappy.algo.structure.KdTree;
import org.behappy.algo.structure.QuadTree;
...
public class SpatialDataStructuresTiming {
    ...
    private static boolean testKdTree(String name, int testRound, int testNum, int[][] unsorteds) {
        ...
        for (KdTree.XYZPoint p : points) {
            ...
            assertTrue("Point not found.", p, tree, r);
        }
        ...
        for (KdTree.XYZPoint p : points) {
            ...
            assertTrue("Point not removed.", p, tree, r);
        }
    ...
    private static String getTestResults(int number, String[] names, long[][] results) {
    ...
    private static <P extends KdTree.XYZPoint> void assertTrue(String msg, P p, KdTree<P> obj, boolean isTrue) {
        String toString = "";
        if (!isTrue)
    ...
    // Assertion which won't call toString on the tree unless the assertion fails
    private static <P extends QuadTree.XYPoint, Q extends QuadTree.XYPoint> void assertTrue(String msg, P p, QuadTree<Q> obj, boolean isTrue) {
        String toString = "";
        if (!isTrue)
            toString = p.toString() + "\n" + "data=[" + stringifiedData + "]\n" + obj.toString();
        Assert.assertTrue(msg + toString, isTrue);
