package com.behappy.algo.graph.test;

import com.behappy.algo.graph.*;
import com.behappy.algo.structure.Graph;
import com.behappy.algo.structure.Graph.Edge;
import com.behappy.algo.structure.Graph.TYPE;
import com.behappy.algo.structure.Graph.Vertex;
...
public class GraphsTest {
    ...
    private static Graph<Integer> makeDirectedGraph(int N, int K, int[] values) {
        final List<Vertex<Integer>> vertices = new ArrayList<>(values.length);
        for (int value : values) vertices.add(new Vertex<>(value, 0));

        final List<Edge<Integer>> edges = new ArrayList<>(values.length);
        ...
        return new Graph<>(TYPE.DIRECTED, vertices, edges);
    }
    ...
    private Map<Graph.Vertex<Integer>, Graph.CostPathPair<Integer>> getIdealUndirectedPath(UndirectedGraph undirected) {
        ...
        }
        {
            ...
            list.add(undirected.e1_3);
            list.add(undirected.e3_6);
            list.add(new Graph.Edge<>(9, undirected.v6, undirected.v5));
            final Graph.CostPathPair<Integer> path = new Graph.CostPathPair<Integer>(cost, list);
    ...
    private Map<Graph.Vertex<Integer>, Graph.CostPathPair<Integer>> getIdealDirectedPath(DirectedGraph directed) {
        ...
        }
        {
            ...
            list.add(directed.e1_3);
            list.add(directed.e3_6);
            list.add(new Graph.Edge<>(9, directed.v6, directed.v5));
            final Graph.CostPathPair<Integer> path = new Graph.CostPathPair<Integer>(cost, list);
    ...
    private Graph.CostPathPair<Integer> getIdealDirectedWithNegWeightsPathPair(DirectedWithNegativeWeights directedWithNegWeights) {
        ...
        list.add(directedWithNegWeights.e4_2);
        list.add(directedWithNegWeights.e2_3);
        return (new Graph.CostPathPair<>(cost, list));
    }
    ...
    public void testDijkstraUndirected() {
        ...
        {   // UNDIRECTED GRAPH
            ...
            for (Graph.Vertex<Integer> v : map1.keySet()) {
                ...
                assertEquals("Dijkstra's shortest path error. path1=" + path1 + " path2=" + path2, path1, path2);
            }
            ...
            assertEquals("Dijkstra's shortest path error. pair=" + pair1 + " pair=" + getIdealUndirectedPathPair(undirected), pair1, getIdealUndirectedPathPair(undirected));
        }
    }
    ...
    public void testBellmanFordUndirected() {
        ...
        final Graph.Vertex<Integer> end = undirected.v5;
        {
            ...
            for (Graph.Vertex<Integer> v : map2.keySet()) {
                ...
                final Graph.CostPathPair<Integer> path2 = getIdealUndirectedPath(undirected).get(v);
                assertEquals("Bellman-Ford's shortest path error. path1=" + path1 + " path2=" + path2, path1, path2);
            }

            final Graph.CostPathPair<Integer> pair2 = BellmanFord.getShortestPath(undirected.graph, start, end);
            assertEquals("Bellman-Ford's shortest path error. pair=" + pair2 + " result=" + getIdealUndirectedPathPair(undirected), pair2, getIdealUndirectedPathPair(undirected));
        }
    }
    ...
    public void testPrimUndirected() {
        ...
        Graph.Vertex<Integer> start = undirected.v1;
        {
            final Graph.CostPathPair<Integer> resultMST = Prim.getMinimumSpanningTree(undirected.graph, start);
            {
                ...
                list.add(undirected.e3_6);
                list.add(new Graph.Edge<>(9, undirected.v6, undirected.v5));
                list.add(new Graph.Edge<>(6, undirected.v5, undirected.v4));
                final Graph.CostPathPair<Integer> idealMST = new Graph.CostPathPair<Integer>(cost, list);

                assertEquals("Prim's minimum spanning tree error. resultMST=" + resultMST + " idealMST=" + idealMST, resultMST, idealMST);
            }
            ...
            final Graph.Edge<Integer> ce1_2 = new Graph.Edge<>(3, cv1, cv2);
            cyclicEdges.add(ce1_2);
            final Graph.Edge<Integer> ce2_3 = new Graph.Edge<>(2, cv2, cv3);
            cyclicEdges.add(ce2_3);
            final Graph.Edge<Integer> ce3_4 = new Graph.Edge<>(4, cv3, cv4);
            cyclicEdges.add(ce3_4);
            final Graph.Edge<Integer> ce4_1 = new Graph.Edge<>(1, cv4, cv1);
            cyclicEdges.add(ce4_1);
            final Graph.Edge<Integer> ce4_5 = new Graph.Edge<>(1, cv4, cv5);
            ...
            final Graph.CostPathPair<Integer> pair4 = Prim.getMinimumSpanningTree(cyclicUndirected, start);
            {
                ...
                final int cost = 7;
                final List<Graph.Edge<Integer>> list = new ArrayList<>();
                list.add(new Graph.Edge<>(1, cv1, cv4));
                list.add(ce4_5);
                list.add(ce1_2);
                list.add(ce2_3);
                final Graph.CostPathPair<Integer> result4 = new Graph.CostPathPair<>(cost, list);

                assertEquals("Prim's minimum spanning tree error. pair4=" + pair4 + " result4=" + result4, pair4, result4);
            }
        }
    }
    ...
    public void testKruskalUndirected() {
        final UndirectedGraph undirected = new UndirectedGraph();
        {
            final Graph.CostPathPair<Integer> resultMST = Kruskal.getMinimumSpanningTree(undirected.graph);
            {
                ...
                list.add(new Graph.Edge<>(9, undirected.v6, undirected.v5));
                list.add(new Graph.Edge<>(6, undirected.v5, undirected.v4));
                final Graph.CostPathPair<Integer> idealMST = new Graph.CostPathPair<>(cost, list);

                assertEquals("Kruskal's minimum spanning tree error. resultMST=" + resultMST + " idealMST=" + idealMST, resultMST.getCost(), idealMST.getCost());
            }
            ...
            final Graph<Integer> cyclicUndirected = new Graph<>(TYPE.UNDIRECTED, cyclicVertices, cyclicEdges);
            ...
            final Graph.CostPathPair<Integer> pair4 = Kruskal.getMinimumSpanningTree(cyclicUndirected);
            {
                ...
                assertEquals("Kruskal's minimum spanning tree error. pair4=" + pair4 + " result4=" + result4, pair4.getCost(), result4.getCost());
            }
        }
    }
    ...
    public void testDijkstraDirected() {
        ...
        // Compare pair
        assertEquals("Dijkstra's shortest path error. pair1=" + pair1 + " idealPathPair=" + getIdealPathPair(directed), pair1, getIdealPathPair(directed));
    }
    ...
    public void testBellmanFordDirected() {
        ...
        assertNotNull("No path from " + start.getValue() + " to " + end.getValue(), pair2);

        // Compare pair
        assertEquals("Bellman-Ford's shortest path error. pair2=" + pair2 + " idealPathPair=" + getIdealPathPair(directed), pair2, getIdealPathPair(directed));
    }
    ...
    public void testDijkstraDirectedWihtNegativeWeights() {
        ...
        {   // DIRECTED GRAPH (WITH NEGATIVE WEIGHTS)
            ...
            assertNotNull("No path from " + start.getValue() + " to " + end.getValue(), pair1);

            // Compare pair
            assertEquals("Bellman-Ford's shortest path error. pair1=" + pair1 + " result2=" + getIdealDirectedWithNegWeightsPathPair(directedWithNegWeights), pair1, getIdealDirectedWithNegWeightsPathPair(directedWithNegWeights));
        }
    }
    ...
    public void testJohnonsonsAllPairsShortestPathOnDirecteWithNegWeights() {
        final DirectedWithNegativeWeights directedWithNegWeights = new DirectedWithNegativeWeights();
        {
            final var path = Johnson.getAllPairsShortestPaths(directedWithNegWeights.graph);
            assertTrue("Directed graph contains a negative weight cycle.", (path != null));

            final Map<Vertex<Integer>, Map<Vertex<Integer>, List<Edge<Integer>>>> result = new HashMap<>();
            {
                {   // vertex 3
                    final Map<Vertex<Integer>, List<Edge<Integer>>> m = new HashMap<>();
                    final List<Edge<Integer>> s3 = new ArrayList<>();
                    m.put(directedWithNegWeights.v3, s3);
                    final List<Edge<Integer>> s4;
                    s4 = new ArrayList<>();
                    s4.add(directedWithNegWeights.e3_4);
                    m.put(directedWithNegWeights.v4, s4);
                    final List<Edge<Integer>> s2 = new ArrayList<>();
                    s2.add(directedWithNegWeights.e3_4);
                    s2.add(directedWithNegWeights.e4_2);
                    m.put(directedWithNegWeights.v2, s2);
                    final List<Edge<Integer>> s1 = new ArrayList<>();
                ...
                {   // vertex 4
                    ...
                    m.put(directedWithNegWeights.v3, s3);
                    final List<Edge<Integer>> s4 = new ArrayList<>();
                    m.put(directedWithNegWeights.v4, s4);
                    final List<Edge<Integer>> s2 = new ArrayList<>();
                    s2.add(directedWithNegWeights.e4_2);
                    m.put(directedWithNegWeights.v2, s2);
            ...
            for (Vertex<Integer> vertex1 : path.keySet()) {
                ...
                for (Vertex<Integer> vertex2 : map1.keySet()) {
                    ...
                    assertEquals("Johnson's all-pairs shortest path error. size3=" + list1.size() + " size4=" + list2.size(), list1.size(), list2.size());

                    while (iter1.hasNext() && iter2.hasNext()) {
                        Edge<Integer> e1 = (Edge<Integer>)iter1.next();
                        Edge<Integer> e2 = (Edge<Integer>)iter2.next();
                        assertEquals("Johnson's all-pairs shortest path error. e1.from=" + e1.getFromVertex() + " e2.from=" + e2.getFromVertex(), e1.getFromVertex(), e2.getFromVertex());
                        assertEquals("Johnson's all-pairs shortest path error. e1.to=" + e1.getToVertex() + " e2.to=" + e2.getToVertex(), e1.getToVertex(), e2.getToVertex());
                    }
                }
            }
        }
        ...
        final DirectedWithNegativeWeights directedWithNegWeights = new DirectedWithNegativeWeights();
        {
            ...
            for (Vertex<Integer> vertex1 : pathWeights.keySet()) {
                ...
                for (Vertex<Integer> v : m1.keySet()) {
                    ...
                    assertEquals("Floyd-Warshall's all-pairs shortest path weights error. i1=" + i1 + " i2=" + i2, i1, i2);
                }
    ...
    public void cycleCheckOnUndirected() {
        ...
        {   // UNDIRECTED GRAPH
            ...
            final List<Edge<Integer>> cycledEdges = new ArrayList<>();
    ...
    public void topologicalSortOnDirectedGraph() {
        {   // DIRECTED GRAPH
            ...
            assertFalse("Topological sort error. results=" + results1, results1.isEmpty());

            final List<Graph.Vertex<Integer>> results2 = new ArrayList<>(results1.size());
            {  // Ideal sort
                results2.add(cv6);
                results2.add(cv3);
                results2.add(cv5);
            ...
            // Compare results
            {
                ...
                assertEquals("Topological sort error. size1=" + results1.size() + " size2=" + results2.size(), results1.size(), results2.size());
                while (iter1.hasNext() && iter2.hasNext()) {
                    ...
                    assertEquals("Topological sort error. v1=" + v1 + " v2=" + v2, v1, v2);
                }
            }
        }
    }
    ...
    public void connectedComponents() {
        {
            ...
            assertEquals(2, ConnectedComponents.getConnectedComponents(g).size());
        }

        {
            final Graph<Integer> g = makeDirectedGraph(5, 1, new int[]{5, 3, 4, 5, 6});
            assertEquals(1, ConnectedComponents.getConnectedComponents(g).size());
        }

        {
            final Graph<Integer> g = makeDirectedGraph(8, 51, new int[]{49, 57, 3, 95, 98, 100, 44, 40});
            assertEquals(3, ConnectedComponents.getConnectedComponents(g).size());
        }
...

        {
            final Graph<Integer> g = makeDirectedGraph(15, 564, new int[]{617, 400, 658, 30, 891, 517, 304, 156, 254, 610, 72, 371,
                    411, 689, 381});
            assertEquals(10, ConnectedComponents.getConnectedComponents(g).size());
        }

        {
            final Graph<Integer> g = makeDirectedGraph(13, 422, new int[]{87, 950, 773, 928, 696, 131, 809, 781, 348, 144, 717, 555, 311});
            assertEquals(6, ConnectedComponents.getConnectedComponents(g).size());
        }
    ...
    public void testAStarUndirected() {
        ...
        {   // UNDIRECTED GRAPH
            final AStar<Integer> aStar = new AStar<>();
            ...
            assertEquals("A* path error. path=" + path + " idealPathPair=" + ideal, path, ideal);
        }
    }
